...
const universities = graduates.map(g => {
  return g.university;
});
...
const contactInfo = graduates.map(g => {
  return `${g.first_name} ${g.email}`;
});

console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter(g => {
  if (g.university.includes("Uni")) {
    return g.university;
  }
});
...
*/
const lowPopulationAnimals = zooAnimals.filter(a => {
  if (a.population < 5) {
    return a;
  }
});
